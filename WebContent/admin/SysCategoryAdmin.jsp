					...
					<tbody>
					 ...
					 %>
						<tr>
		          			...
		          			<td>	<% if (!"无分类".equals(scg.getCategoryName())) { %>
		          						<a href="<%=basePath %>admin.html?action=EditSysCategory&scgId=<%=scg.getId() %>"><i class="glyphicon glyphicon-pencil"></i></a>
		          						<a onClick="dele('<%=deleUrl %>')"><i class="icon-remove"></i></a>
		          					<% } else { %>
		          						<%="系统默认分类"%>
		          					<% } %>
		          			</td>
